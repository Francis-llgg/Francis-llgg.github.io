...
export const projectRecords: ProjectRecord[] = [
  ...
  },
  {
    ...
    en: {
      ...
      evidenceTitle: "Model architecture and fusion path",
      outcomeTitle: "Results and lessons",
      contributions: [
      ...
      results: [
        "Added enhanced radar features, three-sweep accumulation and camera BEV fusion step by step, raising BEV mAP by 34.8% over Radar-only.",
        "Delivered LiDAR-free three-class 3D detection from aligned radar sweeps and monocular RGB, with critical settings stored in each checkpoint.",
        "Kept Top-K checkpoints by validation mAP and compared two independent runs, selecting the stronger model for leaderboard submission.",
      ],
      limitations: "The four-hour training budget ruled out BEVFormer-style Transformer fusion and a learned temporal model; time cues came only from ego-motion-aligned radar sweeps. I also did not first stabilize a strong CenterPoint baseline, so some module gains were hard to isolate. Next, I would lock the baseline, then add fusion and temporal modules one at a time with matched ablations.",
    ...
    zh: {
      ...
      results: [
        "逐步加入增强雷达特征、三帧累积和相机 BEV 融合，最终较 Radar-only 基线提升 34.8% BEV mAP。",
        "完成无 LiDAR 的三类 3D 检测链路：输入对齐后的 Radar Sweep 与单目 RGB，输出汽车、行人和骑行者 3D 框；关键配置随 Checkpoint 保存。",
        "按验证集 mAP 保留 Top-K Checkpoint，并比较两次独立训练结果，选择更优模型提交排行榜。",
      ],
      limitations: "受四小时训练预算限制，本次未采用 BEVFormer 式 Transformer 融合或可学习时序模型，时间信息仅来自运动补偿后的多帧雷达。由于没有先稳定强 CenterPoint 基线，部分模块的独立增益难以判断。后续将先固定基线，再逐项加入融合与时序模块，并用消融实验验证。",
